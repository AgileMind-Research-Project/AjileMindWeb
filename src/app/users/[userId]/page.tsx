...
export default function EditUserPage() {
    ...
    const fetchData = async () => {
        ...
        try {
            ...
            // Handle backward compatibility for stack field
            let stackValue: 'backend' | 'frontend' | 'both' = 'backend';
            if (user.user_data?.stack) {
                if (Array.isArray(user.user_data.stack)) {
                    // Old format: array
                    if (user.user_data.stack.includes('backend') && user.user_data.stack.includes('frontend')) {
                        stackValue = 'both';
                    } else if (user.user_data.stack.includes('frontend')) {
                        stackValue = 'frontend';
                    } else {
                        stackValue = 'backend';
                    }
                    ...
                    // New format: string
                    stackValue = user.user_data.stack as 'backend' | 'frontend' | 'both';
                }
            }
